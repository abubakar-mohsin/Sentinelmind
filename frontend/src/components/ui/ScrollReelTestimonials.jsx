...
/* ----------------------------------------------------------------
 * ScrollReelTestimonials — JSX port of scroll-reel-testimonials.tsx
 *
 * Counter-rotating scroll reel + per-character text rise.
 * Middle column: real vertical list of portraits that translates
 * by one "pitch" per step; outer columns counter-rotate opposite.
 * Text animates in character-by-character with a stagger.
 * ---------------------------------------------------------------- */
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-border bg-gradient-to-b from-secondary to-card blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.06)]"
...
/* Per-character split with staggered animation */
...
export function ScrollReelTestimonials({ testimonials, charStaggerMs = 6, className }) {
  ...
  /* Middle column: 3 leading cells, then featured + 2 gap cells between each */
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        'relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-border bg-muted shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] outline-none focus-visible:ring-2 focus-visible:ring-ring md:min-h-[320px] md:flex-row',
        className
      )}
    >
      {/* ── Reel section ── */}
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center gap-2">
          {/* Left column — counter-rotates opposite to middle */}
          ...
          {/* Middle column — scrolls to center the active portrait */}
          ...
          {/* Right column — counter-rotates opposite to middle */}
          <div className="flex shrink-0 flex-col gap-2 will-change-transform" style={colStyle(sideY)}>
            {Array.from({ length: sideCellCount }).map((_, i) => <Cell key={i} />)}
          </div>
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          {/* Quotation mark */}
          <svg
            className="block h-12 w-12 text-muted-foreground"
            style={{ opacity: 0.4 }}
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          ...
          {/* Text stage — invisible copy sizes the container, animated copy overlays it */}
          <div
          ...
          >
            ...
            <div
              key={displayIndex}
              className={cn(
                'absolute inset-x-0 top-0 flex flex-col gap-[19px]',
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-border bg-transparent p-0 text-foreground transition-[opacity,transform] duration-200 disabled:cursor-default disabled:opacity-40"
          >
            <svg className="h-3 w-3 opacity-70" viewBox="0 0 12 12" fill="none"
